...
import static com.arturoccs.utility.Format.formatDate;
import static com.arturoccs.utility.TemperatureIconMapper.getIconForTemperature;
...
public class HomeScreenController implements Initializable {
    ...
    private void addDataToGrid(Data data, int column, int row) {
        try {
            FXMLLoader fxmlLoader = new FXMLLoader(getClass().getResource("/assets/item.fxml"));
...
//        temperatureUnit.setText("°C");
        ...
        URL resourceUrl = getClass().getResource( iconPath); // Ajusta la ruta según la estructura

            if (resourceUrl != null) {
                Image image = new Image(resourceUrl.toString());
                weatherImg.setImage(image);
            } else {
                // Si no se encuentra el recurso, cargar una imagen por defecto
                Image defaultImage = new Image("/icons/sunny.png");
                weatherImg.setImage(defaultImage);
            }
    ...
    private List<Data> getData(){
    	...
    	com.arturoccs.controller.TxtParser.weatherDataToTxt(lines);
    	
    	try {
			return com.arturoccs.controller.DataParser.txtToData();
		} catch (IOException e) {
