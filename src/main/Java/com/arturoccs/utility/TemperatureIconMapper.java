...
public class TemperatureIconMapper {
    ...
    private static String getIconFromRanges(double temperature, TreeMap<Double, String> ranges) {
        for (Double key : ranges.descendingKeySet()) {
            if (temperature >= key) {
                String iconPath = ranges.get(key);
                return TemperatureIconMapper.class.getResource(iconPath).toExternalForm();
            }
        }
        return TemperatureIconMapper.class.getResource("/icons/extreme_heat.png").toExternalForm(); 
    }
